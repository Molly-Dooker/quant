...
FIRST_RUNS = 3
NUM_RUNS = 10  # or more for better stability
...
if __name__ == '__main__':
    ...
    # For split backbone running
    # ttnn_model.load_config_dict(bevformer_config_split_v55_4x5)
    list_img = [img.clone() for i in range(FIRST_RUNS)]
    list_img_ = [img.clone() for i in range(NUM_RUNS)]
    img_ = img.clone()

    for i in range(FIRST_RUNS):
        print(f"Running inference {i+1}/{FIRST_RUNS} for model compilation")
        ttnn_out = ttnn_model.forward_test(img=[list_img[i]], img_metas=img_metas)
    ttnn.synchronize_device(device)
    start_time = time.time() 
    for i in range(NUM_RUNS):
        print(f"Running inference {i+1}/{NUM_RUNS} for FPS estimation")
        ttnn_model.forward_test(img=[list_img_[i]], img_metas=img_metas)
    ttnn.synchronize_device(device) 
    end_time = time.time()
    avg_time_per_run = (end_time - start_time) / NUM_RUNS
    fps = 1 / avg_time_per_run
    print(f"\nAverage time per inference: {avg_time_per_run:.4f} seconds")
