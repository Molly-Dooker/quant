...
from bevformer.tt.modules.backbone.resnet import ResNet
from bevformer.tt.modules.neck.fpn import FPN
from bevformer.tt.modules.head.bevformer_head import BEVFormerHead
from bevformer.tt.modules.bev import BEVFormer as BEVFormerTTNN
from pathlib import Path

# from bevformer_config_ttnn import bevformer_config_v55_4x5
...
def load_input(path, location='cpu'):
    input_folder = Path("data/test_inputs/")
...
def load_inputs(location='cpu'):
        ...
        return {
            ...
            'l2g_t': load_input('l2g_t.pth', location),
            'l2g_r_mat': load_input('l2g_r_mat.pth', location),
            'timestamp': load_input('timestamp.pth', location),
            'gt_lane_labels': load_input('gt_lane_labels.pth', location),
            'gt_lane_masks': load_input('gt_lane_masks.pth', location),
            'rescale': False,
            # planning gt(for evaluation only)
            'sdc_planning': load_input('sdc_planning.pth', location),
            'sdc_planning_mask': load_input('sdc_planning_mask.pth', location),
            'command': load_input('command.pth', location),
            # Occ_gt (for evaluation only)
            'gt_segmentation': load_input('gt_segmentation.pth', location),
            'gt_instance': load_input('gt_instance.pth', location),
            'gt_occ_img_is_valid': load_input('gt_occ_img_is_valid.pth', location),
            'return_loss': False,
        }
...
if __name__ == '__main__':
    # Test BEVFormer
    device_box.device_config.update({"device_id": 0, "l1_small_size": 48*32*32})
    ...
    # torch_model = build_detector(bevformer_tiny_cfg, test_bev_tiny_cfg)
    # load_result = BEVFormerTorch.load_state_dict(state_dict)
    # BEVFormerTorch.eval()
    # processor = bm.ModelProcessor(BEVFormerTorch, capture_output=True)
    # Test extract_img_feat
    ...
    # img_metas = img_metas.to(torch.bfloat16)
    # save_tensor(img, "input_tensor")
    # kwargs = {'img':img, 
    #           'img_metas':img_metas}
    # with torch.no_grad():
    #     state_dict = processor.process_state_dict(kwargs=kwargs)
    # torch.save(state_dict, "./state_dict/bevformer_tiny_ttnn.pth")
    ...
    # with torch.no_grad():
    #     torch_output = BEVFormerTorch.forward_test(img=img, img_metas=img_metas)
