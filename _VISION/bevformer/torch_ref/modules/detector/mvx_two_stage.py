...
from bevformer.torch_ref.modules.voxel import Voxelization
from bevformer.torch_ref.modules.utils import (build_voxel_encoder, build_middle_encoder, build_backbone, 
                           build_fusion_layer, build_neck, build_head, multi_apply)
from bevformer.torch_ref.modules.detector.bbox import bbox3d2result
from bevformer.torch_ref.modules.registry import DETECTORS
from .base3d import Base3DDetector
...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        if pts_fusion_layer:
            self.pts_fusion_layer = build_fusion_layer(
                pts_fusion_layer)
        ...
        if img_backbone and type(img_backbone) is dict:
            self.img_backbone = build_backbone(img_backbone)
        else:
            self.img_backbone = img_backbone
        if img_neck is not None and type(img_neck) is dict:
