...
from bevformer.torch_ref.bevformer_model import BEVFormer as BEVFormerTorch
...
def load_input(path, location='cpu'):
    ...
    return torch.load(input_folder / path, map_location='cpu')
...
if __name__ == '__main__':
    ...
    # imgs = [plt.imread('/data/sets/nuScenes/' + nusc.get('sample_data', my_sample['data'][sensor])['filename'])/255. for sensor in sensors]
    # imgs = torch.from_numpy(np.array(imgs)).float()
    # imgs = imgs.permute(0,3, 1, 2).unsqueeze(0)
    # img = [imgs]
    ...
    # Random img for optimization
    # img, _ = create_forward_test_inputs()

    # breakpoint()
    # metas = img_metas[0][0]['lidar2img']    
    # img_metas[0][0]['lidar2img'] = metas[0]


    
    with torch.no_grad():
        torch_output = BEVFormerTorch(img=img, img_metas=img_metas)
    print(torch_output)
