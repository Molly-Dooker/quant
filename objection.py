...
def main():
    ...
    if parser.with_tracking:
        accelerator_log_kwargs["log_with"] = parser.report_to
        accelerator_log_kwargs["project_dir"] = parser.output_dir

    accelerator = Accelerator(gradient_accumulation_steps=parser.gradient_accumulation_steps, **accelerator_log_kwargs)
    ...
    if parser.seed is not None:
        set_seed(parser.seed, device_specific=True)

    # Handle the repository creation
    if accelerator.is_main_process:
        if parser.push_to_hub:
            # Retrieve of infer repo_name
            repo_name = parser.hub_model_id
            if repo_name is None:
                repo_name = Path(parser.output_dir).absolute().name
            # Create repo and retrieve repo_id
            api = HfApi()
            repo_id = api.create_repo(repo_name, exist_ok=True, token=parser.hub_token).repo_id

            with open(os.path.join(parser.output_dir, ".gitignore"), "w+") as gitignore:
                if "step_*" not in gitignore:
                    gitignore.write("step_*\n")
                if "epoch_*" not in gitignore:
                    gitignore.write("epoch_*\n")
        elif parser.output_dir is not None:
    ...
    dataset = load_dataset(parser.dataset_name, cache_dir=parser.cache_dir, trust_remote_code=parser.trust_remote_code)

    # If we don't have a validation split, split off a percentage of train as validation.
    parser.train_val_split = None if "validation" in dataset.keys() else parser.train_val_split
    if isinstance(parser.train_val_split, float) and parser.train_val_split > 0.0:
        split = dataset["train"].train_test_split(parser.train_val_split, seed=parser.seed)
    ...
    common_pretrained_args = {
        ...
        "token": parser.hub_token,
        "trust_remote_code": parser.trust_remote_code,
    }
    config = AutoConfig.from_pretrained(
        parser.model_name_or_path, label2id=label2id, id2label=id2label, **common_pretrained_args
    )
    model = AutoModelForObjectDetection.from_pretrained(
        parser.model_name_or_path,
        config=config,
        ignore_mismatched_sizes=parser.ignore_mismatched_sizes,
        **common_pretrained_args,
    )
    image_processor = AutoImageProcessor.from_pretrained(
        parser.model_name_or_path,
        do_resize=True,
        size={"max_height": parser.image_square_size, "max_width": parser.image_square_size},
        do_pad=True,
        pad_size={"height": parser.image_square_size, "width": parser.image_square_size},
        use_fast=parser.use_fast,
        **common_pretrained_args,
    )
    ...
    # ------------------------------------------------------------------------------------------------
    max_size = parser.image_square_size
    ...
    dataloader_common_args = {
        "num_workers": parser.dataloader_num_workers,
        "collate_fn": collate_fn,
    }
    train_dataloader = DataLoader(
        train_dataset, shuffle=True, batch_size=parser.per_device_train_batch_size, **dataloader_common_args
    )
    valid_dataloader = DataLoader(
        valid_dataset, shuffle=False, batch_size=parser.per_device_eval_batch_size, **dataloader_common_args
    )
    test_dataloader = DataLoader(
        test_dataset, shuffle=False, batch_size=parser.per_device_eval_batch_size, **dataloader_common_args
    )
    ...
    optimizer = torch.optim.AdamW(
        ...
        betas=[parser.adam_beta1, parser.adam_beta2],
        eps=parser.adam_epsilon,
    )

    # Figure out how many steps we should save the Accelerator states
    checkpointing_steps = parser.checkpointing_steps
    if checkpointing_steps is not None and checkpointing_steps.isdigit():
        checkpointing_steps = int(checkpointing_steps)

    # Scheduler and math around the number of training steps.
    overrode_max_train_steps = False
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / parser.gradient_accumulation_steps)
    if parser.max_train_steps is None:
        parser.max_train_steps = parser.num_train_epochs * num_update_steps_per_epoch
        overrode_max_train_steps = True

    lr_scheduler = get_scheduler(
        name=parser.lr_scheduler_type,
        optimizer=optimizer,
        num_warmup_steps=parser.num_warmup_steps * accelerator.num_processes,
        num_training_steps=parser.max_train_steps
        if overrode_max_train_steps
        else parser.max_train_steps * accelerator.num_processes,
    )
    ...
    parser.num_train_epochs = math.ceil(parser.max_train_steps / num_update_steps_per_epoch)

    # We need to initialize the trackers we use, and also store our configuration.
    # The trackers initializes automatically on the main process.
    if parser.with_tracking:
        experiment_config = vars(parser)
    ...
    total_batch_size = parser.per_device_train_batch_size * accelerator.num_processes * parser.gradient_accumulation_steps

    logger.info("***** Running training *****")
    logger.info(f"  Num examples = {len(train_dataset)}")
    logger.info(f"  Num Epochs = {parser.num_train_epochs}")
    logger.info(f"  Instantaneous batch size per device = {parser.per_device_train_batch_size}")
    logger.info(f"  Total train batch size (w. parallel, distributed & accumulation) = {total_batch_size}")
    logger.info(f"  Gradient Accumulation steps = {parser.gradient_accumulation_steps}")
    logger.info(f"  Total optimization steps = {parser.max_train_steps}")

    # Only show the progress bar once on each machine.
    progress_bar = tqdm(range(parser.max_train_steps), disable=not accelerator.is_local_main_process)
    completed_steps = 0
    starting_epoch = 0

    # Potentially load in the weights and states from a previous save
    if parser.resume_from_checkpoint:
        if parser.resume_from_checkpoint is not None or parser.resume_from_checkpoint != "":
            checkpoint_path = parser.resume_from_checkpoint
            path = os.path.basename(parser.resume_from_checkpoint)
        else:
        ...
        if "epoch" in training_difference:
            ...
            resume_step = int(training_difference.replace("step_", "")) * parser.gradient_accumulation_steps
            starting_epoch = resume_step // len(train_dataloader)
            completed_steps = resume_step // parser.gradient_accumulation_steps
            resume_step -= starting_epoch * len(train_dataloader)

    # update the progress_bar if load from checkpoint
    progress_bar.update(completed_steps)

    for epoch in range(starting_epoch, parser.num_train_epochs):
        model.train()
        if parser.with_tracking:
            total_loss = 0
        if parser.resume_from_checkpoint and epoch == starting_epoch and resume_step is not None:
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0 and accelerator.sync_gradients:
                    ...
                    if parser.output_dir is not None:
                        output_dir = os.path.join(parser.output_dir, output_dir)
                    accelerator.save_state(output_dir)

                    if parser.push_to_hub and epoch < parser.num_train_epochs - 1:
                        accelerator.wait_for_everyone()
                        ...
                        if accelerator.is_main_process:
                            image_processor.save_pretrained(parser.output_dir)
                            api.upload_folder(
                                commit_message=f"Training in progress epoch {epoch}",
                                folder_path=parser.output_dir,
                                repo_id=repo_id,
                                repo_type="model",
                                token=parser.hub_token,
                            )

            if completed_steps >= parser.max_train_steps:
                break
        ...
        if parser.push_to_hub and epoch < parser.num_train_epochs - 1:
            ...
            unwrapped_model = accelerator.unwrap_model(model)
            unwrapped_model.save_pretrained(
                parser.output_dir, is_main_process=accelerator.is_main_process, save_function=accelerator.save
            )
            if accelerator.is_main_process:
                image_processor.save_pretrained(parser.output_dir)
                api.upload_folder(
        ...
        if parser.checkpointing_steps == "epoch":
    ...
    if parser.output_dir is not None:
        ...
        if accelerator.is_main_process:
            with open(os.path.join(parser.output_dir, "all_results.json"), "w") as f:
