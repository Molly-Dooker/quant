...
def _choose_qparams_per_token_symmetric_impl(
    input: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    qmin, qmax = -128, 127
    max_val = torch.amax(input.abs(), dim=-1, keepdim=True)
    eps = torch.finfo(torch.float32).eps
    scale = max_val / (float(qmax - qmin)/2)
    scale = scale.clamp(min=eps)
    zero_point = torch.zeros_like(scale,dtype=torch.int64)     

    return scale.to(torch.float64), zero_point
...
def wrap_module_forward_quantized(module: Module, scheme: QuantizationScheme):
    ...
    def wrapped_forward(self, *args, **kwargs):
        ...
        if scheme.input_activations is not None:
            ...
            DTYPE = input_.dtype
            scale, zp = _choose_qparams_per_token_symmetric_impl(input_) 
            input_    = quantize_per_token(input_,scale,zp,-128,127, torch.int8)
            input_   = dequantize_per_token(input_,scale,zp,DTYPE)
