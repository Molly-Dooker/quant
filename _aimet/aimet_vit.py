...
def get_model(source:str,device:str):
    ...
    if source=='hf':
        ...
        model = ViTForImageClassification.from_pretrained("google/vit-base-patch16-224",device_map=device)
...
def run(model:str, device:str, prefix:str, adaround:bool, cle:bool, evaluate_base=False, config_file='aimet_config_ch.json'):
    ...
    logger.info(f"{model}, {device}, {prefix}, adaroud:{adaround}, cle:{cle}")
    ...
    if not (cle or adaround):
        # model2 = ViTForImageClassification.from_pretrained("google/vit-base-patch16-224",device_map=device)
        ...
        sim = QuantizationSimModel(model=model,
                                ...
                                default_param_bw=8,
                                config_file=config_file)
        # sim.model.classifier.input_quantizers[0]=None
        # sim.model.classifier.param_quantizers['weight']=None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cle', action='store_true', help='Enable CLE mode')
    parser.add_argument('--no-cle', dest='cle', action='store_false', help='Disable CLE mode')
    parser.set_defaults(cle=False)
    parser.add_argument('--adaround', action='store_true', help='Enable ADA round')
    parser.add_argument('--no-adaround', dest='adaround', action='store_false', help='Disable ADA round')
    parser.set_defaults(adaround=False)
